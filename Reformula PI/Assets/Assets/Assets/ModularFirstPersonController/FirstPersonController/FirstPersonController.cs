...
public class FirstPersonController : MonoBehaviour
{
    ...
    public Vector3 directionToAutoRun;
    ...
    private void HandBob()
    {
        if (isWalking)
        {
            ...
            if (isSprinting)
            {
                handTimer += Time.deltaTime * (handBobSpeed + sprintSpeed);
            }
            // Calculates HeadBob speed during crouched movement
            else if (isCrouched)
            {
                handTimer += Time.deltaTime * (handBobSpeed * speedReduction);
            }
            // Calculates HeadBob speed during walking
            else
            {
                handTimer += Time.deltaTime * handBobSpeed;
            }
            // Applies HeadBob movement
            handJoint.localPosition = new Vector3(handJointOriginalPos.x + Mathf.Sin(handTimer) * handBobAmount.x, handJointOriginalPos.y + Mathf.Sin(handTimer) * handBobAmount.y, handJointOriginalPos.z + Mathf.Sin(handTimer) * handBobAmount.z);
        }
        else
        {
            // Resets when play stops moving
            timer = 0;
            handJoint.localPosition = new Vector3(Mathf.Lerp(handJoint.localPosition.x, handJointOriginalPos.x, Time.deltaTime * handBobSpeed), Mathf.Lerp(handJoint.localPosition.y, handJointOriginalPos.y, Time.deltaTime * handBobSpeed), Mathf.Lerp(handJoint.localPosition.z, handJointOriginalPos.z, Time.deltaTime * handBobSpeed));
        }
    }
}
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.isAutoRunning = EditorGUILayout.ToggleLeft(new GUIContent("Auto Run", "Determines if the player will automatically run."), fpc.isAutoRunning);

        GUI.enabled = fpc.playerCanMove;
        fpc.walkSpeed = EditorGUILayout.Slider(new GUIContent("Walk Speed", "Determines how fast the player will move while walking."), fpc.walkSpeed, .1f, fpc.sprintSpeed);
        fpc.directionToAutoRun = EditorGUILayout.Vector3Field(new GUIContent("Auto Run Direction", "Determines the direction the player will run in while auto running."), fpc.directionToAutoRun);
        GUI.enabled = true;
        ...
        EditorGUILayout.Space();
        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
        GUILayout.Label("HAND Bob Setup", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();

        fpc.enableHandBob = EditorGUILayout.ToggleLeft(new GUIContent("Enable Head Bob", "Determines if the camera will bob while the player is walking."), fpc.enableHandBob);


        GUI.enabled = fpc.enableHandBob;
        fpc.handJoint = (Transform)EditorGUILayout.ObjectField(new GUIContent("Camera Joint", "Joint object position is moved while head bob is active."), fpc.handJoint, typeof(Transform), true);
        fpc.handBobSpeed = EditorGUILayout.Slider(new GUIContent("Speed", "Determines how often a bob rotation is completed."), fpc.handBobSpeed, 1, 20);
        fpc.handBobAmount = EditorGUILayout.Vector3Field(new GUIContent("Bob Amount", "Determines the amount the joint moves in both directions on every axes."), fpc.handBobAmount);
        GUI.enabled = true;
